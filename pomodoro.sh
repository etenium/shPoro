...
while true; do
    ...
    if [ "$countdowns_before_longer_break" -gt 0 ] && [ $((countdowns_completed % countdowns_before_longer_break)) -eq 0 ]; then
    ...
    if [ "$start_break" = "" ] || [ "$start_break" = "y" ] || [ "$start_break" = "Y" ]; then
        ...
        if [ "$restart" = "" ] || [ "$restart" = "y" ] || [ "$restart" = "Y" ]; then
            echo -e "${BLUE}${BOLD}Restarting countdown and break...${RESET}"
        else
            break
        fi
